...
//       .then(res => {
//         console.log(res)
//         let categories = res.category_list.categories;
//         let radioCats = categories.map(cat => 
//           `
//             <div class="action-cat-item">
//               <input type="radio" value="${cat.id}" name="catId" id="${cat.id}"><label for="${cat.id}">${cat.name}</label><br>
//             </div>
//           `
//         )
//         .reduce((a, b) => a + b, "")
...
//       })
//       .then(cats => {
//         document.getElementById("action-cat").innerHTML = cats
//       })
//   },
...
function renderRadioCategory() {
  fetch(
    ...
    .then(res => {
      console.log(res)
      let categories = res.category_list.categories;
      let radioCats = categories.map((cat, index) => 
        `
          <div class="action-cat-item">
            <input 
            type="radio" 
...
export default Component.extend(KeyEnterEscape, {
  ...
  actions: {
    sendTopic() {
      let title = document.getElementById("title-create-input").value
      let content = document.querySelector("#content-block-input textarea").value
      let catId = document.querySelector('input[name="catId"]:checked').value;

      let api_key = "0f5122b30358f96c55f1364c3f5ab5f70b70ee8b8575e88f5cd48f996a1a8c34"
      ...
      var details = {
        ...
        'is_warning': false,
        'archetype': 'regular',
        'typing_duration_msecs': 400,
        'composer_open_duration_msecs': 9000,
        'shared_draft': false,
        'draft_key': 'new_topic',
        'create_as_qa': true,
        'nested_post': true
      ...
      .then(_ => alert("Create OK!"))
      .catch(e => alert(e.json()))
    }
  }
})
